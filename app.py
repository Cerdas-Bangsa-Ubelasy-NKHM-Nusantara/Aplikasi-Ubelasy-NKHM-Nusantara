...
if not st.session_state.splash_selesai:
    ...
    with splash_holder.container():
        ...
        with col_tengah:
            ...
            st.markdown("<p style='text-align: center; font-size: 18px;'>Aplikasi Sistem Pinjaman Model Ubelasy Berbasis PSH<br>+ Aplikasi gaming 4 Kecerdasan (IQ, EQ, SQ, AQ) + Nasionalisme<br>Berbasis Perkembangan Data Personal</p>", unsafe_allow_html=True)
            st.markdown("""
            <style>
            div.stButton > button {
...
# ========== TOMBOL CATATAN PRIBADI (hanya NKHM) ==========
if app_mode == "🌿 NKHM Nusantara (Gamifikasi)":
    ...
    st.sidebar.link_button("📝 Catatan Pribadi", target_url, use_container_width=True)
    st.sidebar.markdown("---")
