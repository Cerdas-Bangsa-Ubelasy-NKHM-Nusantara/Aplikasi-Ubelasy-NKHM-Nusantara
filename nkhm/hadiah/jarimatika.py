...
def show_jarimatika():
    ...
    <div style="background: linear-gradient(135deg, #1a3c6e 0%, #2e7daf 100%);
        ...
        <div style="display: flex; align-items: center; gap: 15px;">
            ...
            <div>
                <div style="font-size: 18px; font-weight: bold;">JARIMATIKA PMD – Computer Vision</div>
                <div style="font-size: 14px; opacity: 0.9;">
                    Perkalian 6-10 dengan deteksi jari dari kamera
                </div>
            </div>
        </div>
    </div>
    ...
    tab1, tab2, tab3 = st.tabs(["🧮 Latihan (CV)", "📖 Panduan", "📊 Riwayat"])
    with tab1:
        show_latihan_cv()
    with tab2:
        show_panduan()
...
def show_latihan_cv():
    try:
        ...
        st.error(f"Error: {e}")
        logging.error(f"show_latihan_cv: {e}")
