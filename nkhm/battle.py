...
def safe_rerun():
    ...
    try:
        ...
        logging.warning(f"st.rerun gagal di battle: {e}")
...
def init_battle_state():
    try:
        ...
        logging.error(f"Error init_battle_state: {e}")
...
def reset_battle():
    try:
        ...
        st.session_state.battle_finished = False
        st.session_state.battle_answers = []
        logging.info("Battle state direset")
    except Exception as e:
        logging.error(f"Error reset_battle: {e}")
...
def update_timer():
    try:
        ...
        logging.error(f"Error update_timer: {e}")
...
def show_battle():
    try:
        ...
        if not st.session_state.battle_active and not st.session_state.battle_finished:
            ...
            # Gunakan form untuk setup battle
            with st.form(key="battle_setup_form"):
                ...
                start_btn = st.form_submit_button("🚀 MULAI PERTANDINGAN", use_container_width=True)
                ...
                if start_btn:
                    try:
                        ...
                        st.error(f"Error memulai pertandingan: {e}")
        ...
        # ========== PERTANDINGAN BERLANGSUNG ==========
            ...
            try:
                ...
                timer_progress = st.session_state.battle_time_left / st.session_state.battle_total_time if st.session_state.battle_total_time > 0 else 0
                ...
                # Gunakan form untuk jawaban agar lebih stabil
                with st.form(key=f"battle_answer_form_{st.session_state.battle_current_q_index}_{st.session_state.battle_current_player}"):
                    ...
                    submit_btn = st.form_submit_button("✅ JAWAB", use_container_width=True)
                    ...
                    if submit_btn:
                        try:
                            ...
                            st.error(f"Error memproses jawaban: {e}")
            except Exception as e:
                logging.error(f"Error during battle: {e}")
                st.error(f"Error dalam pertandingan: {e}")
        ...
        # ========== PERTANDINGAN SELESAI ==========
            ...
            try:
                ...
                logging.error(f"Error showing battle results: {e}")
                st.error(f"Error menampilkan hasil: {e}")
    
    except Exception as e:
        logging.error(f"Error di show_battle: {e}", exc_info=True)
        st.error(f"❌ Terjadi error di Mode Tanding: {e}")
        st.exception(e)
