...
def check_all_sides():
    ...
    if violation_left:
        if "tawanan merusak perbekalan" in violation_left:
    ...
    if violation_right:
        if "tawanan merusak perbekalan" in violation_right:
...
# Pesan untuk kegagalan karena aturan 1 (tawanan + perbekalan)
...
# Pesan untuk kegagalan karena aturan 2 (tawanan + anak)
...
# Pesan untuk kegagalan karena langkah pertama salah
def get_failure_first_step_message(violation_type):
    if "tawanan merusak perbekalan" in violation_type:
        return get_failure_rule1_message()
    else:
        return get_failure_rule2_message()
...
def travel(entitas1, entitas2):
    ...
    # Pengecekan: Apakah ini langkah pertama yang salah?
    ...
    # Jika langkah pertama dan tidak membawa Tawanan (termasuk sendirian), maka GAGAL
    if is_first_step and entitas1 != "tawanan":
        # Cek pelanggaran yang akan terjadi
        left_side = state["left"].copy()
        # Hapus pahlawan dari sisi kiri (karena pahlawan akan pindah)
        left_side["pahlawan"] = False
        violation = check_violation(left_side)
        
        state["game_over"] = True
        if violation and "tawanan merusak perbekalan" in violation:
            state["message"] = f"❌ GAGAL! Di sisi awal, {violation}\n\n{get_failure_rule1_message()}"
        elif violation and "tawanan dan anak buah bertarung duel" in violation:
            state["message"] = f"❌ GAGAL! Di sisi awal, {violation}\n\n{get_failure_rule2_message()}"
        else:
            state["message"] = f"❌ GAGAL! Langkah pertama yang salah!\n\n{get_failure_rule1_message()}"
        return
...
def show_buttons():
    ...
    # Tampilkan pesan
    if state["message"] and not state["game_over"]:
...
def show_river_game():
    ...
    **Aturan:**
    ...
    - **Langkah pertama HARUS membawa Tawanan** (jika tidak, permainan GAGAL).
