...
def safe_rerun():
    ...
    try:
        ...
        logging.warning(f"st.rerun gagal di stomata_pilihan_benar_salah: {e}")
...
def reset():
    try:
        ...
        st.session_state.pbs_submitted = False
        st.session_state.pbs_results = None
        logging.info("PBS state direset")
    except Exception as e:
...
def calculate_result():
    try:
        ...
        if not qlist:
            ...
            return False
        if len(st.session_state.pbs_answers) < len(qlist):
...
def show_pilihan_benar_salah():
    try:
        ...
        logging.error(f"Error di show_pilihan_benar_salah: {e}", exc_info=True)
        st.error(f"❌ Terjadi error di Pilihan Benar/Salah: {e}")
        st.exception(e)
