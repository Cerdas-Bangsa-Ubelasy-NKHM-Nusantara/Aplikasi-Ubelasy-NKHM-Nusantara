...
def safe_rerun():
    ...
    try:
        ...
        logging.warning(f"st.rerun gagal di stomata_pilihan_ganda: {e}")
...
def tentukan_posisi(persen_kasih, persen_iman, persen_pengharapan):
    try:
        ...
        logging.error(f"Error tentukan_posisi: {e}")
        return [10]
    ...
    try:
        ...
        for f in base.glob("*.json"):
            try:
                ...
                logging.warning(f"JSON decode error di {f}: {e}")
                st.warning(f"⚠️ File {f.name} format JSON tidak valid.")
            except Exception as e:
                logging.warning(f"Error membaca {f}: {e}")
                st.warning(f"⚠️ Gagal baca {f.name}: {e}")
        return all_q
    except Exception as e:
        logging.error(f"Error load_questions: {e}")
        return []
...
def get_random_33():
    try:
        ...
        if len(kasih) < 11 or len(iman) < 11 or len(pengharapan) < 11:
            st.warning(f"Jumlah soal tidak mencukupi: Kasih={len(kasih)}, Iman={len(iman)}, Pengharapan={len(pengharapan)} (min 11 masing-masing)")
            return []
        ...
        logging.error(f"Error get_random_33: {e}")
        return []
...
def refresh_questions():
    try:
        ...
        logging.error(f"Error refresh_questions: {e}")
        st.error(f"Error refresh soal: {e}")

def calculate_result():
    try:
        qlist = st.session_state.pg_questions
        if not qlist:
            st.error("Tidak ada soal.")
            return False
        if len(st.session_state.pg_answers) < len(qlist):
            st.error(f"Jawab semua {len(qlist)} soal terlebih dahulu!")
            return False
        ...
        logging.error(f"Error calculate_result: {e}")
        st.error(f"Error menghitung hasil: {e}")
        return False
...
def show_pilihan_ganda():
    try:
        ...
        for i, q in enumerate(qlist):
            with st.container():
                ...
                selected_full = st.radio(
                    ...
                    q['pilihan'],
                    index=default_index,
                    key=f"pg_{i}_{q.get('id', i)}_{st.session_state.pg_refresh_counter}",
                    horizontal=True,
        ...
        total = len(qlist)
        st.progress(terjawab / total if total > 0 else 0, text=f"Progress: {terjawab}/{total} soal terjawab")
            ...
            try:
                ...
                if img_path.exists():
                    try:
                        ...
                        logging.error(f"Error menampilkan gambar: {e}")
                        st.warning("⚠️ Gagal menampilkan gambar Stomata Hati")
                else:
                    st.warning("⚠️ Gambar Stomata Hati belum tersedia.")
                ...
                logging.error(f"Error menampilkan hasil: {e}")
                st.error(f"Error menampilkan hasil: {e}")

    except Exception as e:
        logging.error(f"Error di show_pilihan_ganda: {e}", exc_info=True)
        st.error(f"❌ Terjadi error di Pilihan Ganda: {e}")
        st.exception(e)
